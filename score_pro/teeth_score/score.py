...
class Teeth_Grade():
    ...
    def score_aa2(self, dst_all_mark, dst_fill_mark, site, operation_time):
        ...
        if (self.aa2.CENTER_BIAS_SUBTRACT_START < ratio < self.aa2.CENTER_BIAS_SUBTRACT_START_1):
            center_point_scores = full_scores - cut_scores
        elif (self.aa2.CENTER_BIAS_SUBTRACT_START_1 < ratio < self.aa2.CENTER_BIAS_SUBTRACT_START_2):
            center_point_scores = full_scores - cut_scores*2
        elif (self.aa2.CENTER_BIAS_SUBTRACT_START_2 < ratio < self.aa2.CENTER_BIAS_SUBTRACT_START_3):
        ...
        if contours:
            ...
            # ××××××××××××××××××××××××××××× 反作用到AA1指标 ×××××××××××××××××××××××××
            self.aa1.contains_neighbor *= my_limit(((self.aa2.area_ratio+1)/self.aa2.AREA_RATIO_SCORE) * self.aa1.AREA_K, 0, 1)
            self.aa1.sum()
    ...
    def score_bb1(self, gray_img, mark_img, operation_time):
        ...
        caries_point_thresh = np.mean(gray_img[mark_img == 255])-50              # 灰度阈值
        ...
        # 计算黑点数量
        caries_point_num = np.sum(black_point_show == 255)
        # 计算最下灰度值
        if caries_point_num != 0:
            min_gray_value = np.min(gray_img[black_point_show == 255])
        ...
        if min_gray_value < caries_point_thresh:
            black_level_score -= (((caries_point_thresh - min_gray_value) // self.bb1.BLACK_DEPTH_SUBTRACT_VALUE)+1) \
    ...
    def score_bb3(self, src_image, fill_mark, other_mark, operation_time):
        ...
        if self.bb3.roi_site[0] != -1:
            ...
            #     self.bb3.sum()
            #     return
            if np.sum(fill_mark[roi_row:roi_row+roi_h, roi_col:roi_col+roi_w] !=0) != 0:
                ...
                fill_h_avr = 0
                fill_s_avr = 0
            if np.sum(other_mark[roi_row_other:roi_row_other+roi_h_other, roi_col_other:roi_col_other+roi_w_other] !=0) != 0:
    ...
    def score_all(self, teeth_pro):
        ...
        if is_ok == 0:
            ...
            return 0
        # 先评分BB3，再评分BB2/bb1
        if teeth_pro.neighbor_ok != False or teeth_pro.img_info.operation_time != "术后":
            # print("!!!!!!!!!!!!!!!!!!!!!")
    ...
    def creat_score_txt(self, img_info):
        if os.access(os.path.join(img_info.pro_path, 'score.txt'), os.F_OK):
            f = open(os.path.join(img_info.pro_path, 'score.txt'), 'a', encoding = 'utf-8')
            f.write("\n")
            ...
            print("首次，需创建TXT文件")
            f = open(os.path.join(img_info.pro_path, 'score.txt'), 'w', encoding = 'utf-8')
