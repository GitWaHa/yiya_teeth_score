...
def get_roi(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if PointEnd[0] != PointStart[0] and PointEnd[1] != PointStart[
            ...
            # 提取ROI
            image_roi = image[PointLU[1]:PointRD[1], PointLU[0]:
                              PointRD[0]]  # 先y再x
...
while (1):
    ...
    # 保存图片
    if key == ord('s'):
        print('\n[INFO] Save image time ' +
              datetime.datetime.now().strftime("%H:%M:%S"))
        save_cnt += 1
        if mode == 0:
            cv2.imwrite(
                outputPath + 'src_' + time_now + '_' + str(save_cnt) + '.png',
                image_roi_bak)
            cv2.imwrite(
                outputPath + 'mask_' + time_now + '_' + str(save_cnt) + '.png',
                mask)
            cv2.imwrite(
                outputPath + 'gray_' + time_now + '_' + str(save_cnt) + '.png',
                cv2.cvtColor(image_roi_bak, cv2.COLOR_BGR2GRAY))
        elif mode == 1:
            cv2.imwrite(
                './test_data/' + 'test_' + time_now + '_' + str(save_cnt) +
                '.png', cv2.cvtColor(image_roi_bak, cv2.COLOR_BGR2GRAY))
            cv2.imwrite(
                './test_data/' + 'src_' + time_now + '_' + str(save_cnt) +
                '.png', image_roi_bak)
    # 清除标记
    if key == ord('r'):
        print('[INFO] Back roi')
