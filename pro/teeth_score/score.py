...
class Teeth_Grade():
    ...
    def score_aa1(self, dst_all_mark, site, radius):
        ...
        if left_tooth_point_num > roi_height / 4:
            if min_col > roi_width / 2:
                left_tooth_status = 1
        ...
        if right_tooth_point_num > roi_height / 4:
            if src_col - max_col > roi_width / 2:
    ...
    def score_aa2(self, dst_all_mark, dst_fill_mark, site, operation_time):
        ...
        if operation_time == '术前':  # 术前
            full_scores = 6      # 满分
            cut_scores = 1.5     # 按比例扣分
        else:                    # 术中和术后
            full_scores = 4
            cut_scores = 1
        if ratio > 0.1:
            # 考虑四舍五入， 偏10%~15%扣1.5分，以此类推，扣完为止
            center_point_scores = full_scores - int((ratio - 0.1) / 0.05 + 0.5) * cut_scores
        ...
        if contours:
            ...
            if 0.6 <= area_ratio <= 0.7:
                ratio_diff = 0
            elif area_ratio < 0.6:
                ratio_diff = 0.6 - area_ratio
            else:
                ratio_diff = area_ratio - 0.7

            self.aa2.area_ratio = my_limit(4 - math.ceil(ratio_diff / 0.07) * 1, 0, 4)  # 按比例扣分
    ...
    def score_aa3(self, img_path):
        ...
        if abs((img_col / img_row) - (16 / 9)) > 0.03:
            self.aa3.img_ratio = 0
            # return 0
        else:
            self.aa3.img_ratio = 4

        # 分辨率得分
        print('AA3[INFO] 分辨率（标准1049088）', (img_col * img_row))
        if img_col * img_row >= self.aa3.MIN_RESOLUTION:
            self.aa3.img_resolution = 2
        elif img_col * img_row + 100000 >= self.aa3.MIN_RESOLUTION:
            self.aa3.img_resolution = 1
        else:
            self.aa3.img_resolution = 0
    ...
    def score_bb1(self, gray_img, mark_img, operation_time):
        ...
        black_num_score -= ((caries_point_num + 25) // 50)
        ...
        if min_gray_value < 255:
            black_level_score -= ((caries_point_thresh - min_gray_value) // 10)  # 每低于阈值灰度值扣一分
    ...
    def score_bb4(self, src_gray_img, fill_mark, operation_time, operation_name):
        ...
        if contours:
            ...
            grain_show = np.zeros((100, 100), dtype=np.uint8)
            gap_point_num = 0
            for r in range(fill_gray.shape[0]):
                for c in range(fill_gray.shape[1]):
                    if fill_bin[r][c] == 255 and edge_output[r][c] == 255:
                        grain_show[r][c] = 255
                        gap_point_num += 1
            print("BB4[INFO]:沟壑匹配度", gap_point_num/self.bb4.THR_GAP_NUM)
            # 纹路可视化
            # cv2.imshow("纹路", grain_show)

            if gap_point_num >= self.bb4.THR_GAP_NUM*0.7:
                texture_scores = 10
            else:
                texture_scores = int((gap_point_num / self.bb4.THR_GAP_NUM * 10)*1.5 - 0.5)
