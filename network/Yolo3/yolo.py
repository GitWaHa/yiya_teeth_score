...
from network.Yolo3.model import yolo_eval, yolo_body, tiny_yolo_body
from network.Yolo3.utils import letterbox_image
import os
...
class YOLO(object):
    _defaults = {
        "model_path": os.path.join(work_floder, 'mode/yolov3/yolo.h5'),
        "anchors_path": os.path.join(work_floder,
                                     'mode/yolov3/yolo_anchors.txt'),
        "classes_path": os.path.join(work_floder,
                                     'mode/yolov3/yolo_classes.txt'),
        "score": 0.1,
        "iou": 0.45,
    ...
    def detect_teeth(self, image):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            bottom = min(image.shape[1],
                         np.floor(bottom + 0.5).astype('int32'))
            right = min(image.shape[0], np.floor(right + 0.5).astype('int32'))
            # print(predicted_class, type(score), (type(left), top), (right, bottom))
            result.append([predicted_class, score, left, top, right, bottom])

            cv2.rectangle(image_copy, (left, top), (right, bottom), 255, 1)
